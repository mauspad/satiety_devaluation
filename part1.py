...
expName = 'sat_deval_part1'  # from the Builder filename that created this script
expInfo = {
    'participant': '',
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Silver\\Box\\psychopy_git_masters\\satiety_deval\\part1.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    choose_salt_text = visual.TextStim(win=win, name='choose_salt_text',
        text="Choose your favorite salty reward: \n(use buttons '1', '2', or '3')",
        font='Open Sans',
        pos=(0, 7), height=1.0, wrapWidth=20.0, ori=0.0, 
    ...
    choose_sweet_text = visual.TextStim(win=win, name='choose_sweet_text',
        text="Choose your favorite sweet reward: \n(use buttons '1', '2', or '3')",
        font='Open Sans',
        pos=(0, 7), height=1.0, wrapWidth=20.0, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    not_win = visual.TextStim(win=win, name='not_win',
        text='you did not win a snack',
        font='Open Sans',
        pos=(0, 0), height=1.0, wrapWidth=20.0, ori=0.0, 
        color=[1.0000, 0.2941, -1.0000], colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-1.0);
    left_outcome = visual.ImageStim(
        win=win,
        name='left_outcome', 
        image='default.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), size=(7.5, 7.5),
    ...
    right_outcome = visual.ImageStim(
        win=win,
        name='right_outcome', 
        image='default.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), size=(7.5, 7.5),
    ...
    trials_block = data.TrialHandler(nReps=5.0, method='sequential', 
    ...
    for thisTrials_block in trials_block:
        ...
        trials = data.TrialHandler(nReps=1.0, method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions('conditions/trials_part1.xlsx'),
        ...
        for thisTrial in trials:
            ...
            continueRoutine = True
            # update component parameters for each repeat
            thisExp.addData('outcome.started', globalClock.getTime())
            ...
            if key_resp.keys == 'left':
                ...
                if left_ref == 0:
                    left_outcome.opacity = 0
                    right_outcome.opacity = 0
                ...
                if left_ref == 1:
                    ...
                    thisExp.addData('outcome', '1')
                    rightcount += 1
            not_win.setOpacity(None)
            left_outcome.setOpacity(None)
            left_outcome.setImage(leftstim)
            right_outcome.setOpacity(None)
            right_outcome.setImage(rightstim)
            # keep track of which components have finished
            outcomeComponents = [not_win, left_outcome, right_outcome]
            ...
            while continueRoutine and routineTimer.getTime() < 1.5:
                ...
                # if not_win is starting this frame...
                if not_win.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    not_win.frameNStart = frameN  # exact frame index
                    not_win.tStart = t  # local t and not account for scr refresh
                    not_win.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(not_win, 'tStartRefresh')  # time at next scr refresh
                    # update status
                    not_win.status = STARTED
                    not_win.setAutoDraw(True)
                
                # if not_win is active this frame...
                if not_win.status == STARTED:
                    # update params
                    pass
                
                # if not_win is stopping this frame...
                if not_win.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > not_win.tStartRefresh + 1.5-frameTolerance:
                        # keep track of stop time/frame for later
                        not_win.tStop = t  # not accounting for scr refresh
                        not_win.frameNStop = frameN  # exact frame index
                        # update status
                        not_win.status = FINISHED
                        not_win.setAutoDraw(False)
                
                # *left_outcome* updates
                
                # if left_outcome is starting this frame...
                if left_outcome.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    left_outcome.frameNStart = frameN  # exact frame index
                    left_outcome.tStart = t  # local t and not account for scr refresh
                    left_outcome.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(left_outcome, 'tStartRefresh')  # time at next scr refresh
                    # update status
                    left_outcome.status = STARTED
                    left_outcome.setAutoDraw(True)
                
                # if left_outcome is active this frame...
                if left_outcome.status == STARTED:
                    # update params
                    pass
                
                # if left_outcome is stopping this frame...
                if left_outcome.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > left_outcome.tStartRefresh + 1.5-frameTolerance:
                        # keep track of stop time/frame for later
                        left_outcome.tStop = t  # not accounting for scr refresh
                        left_outcome.frameNStop = frameN  # exact frame index
                        # update status
                        left_outcome.status = FINISHED
                        left_outcome.setAutoDraw(False)
                
                # *right_outcome* updates
                
                # if right_outcome is starting this frame...
                if right_outcome.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    right_outcome.frameNStart = frameN  # exact frame index
                    right_outcome.tStart = t  # local t and not account for scr refresh
                    right_outcome.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(right_outcome, 'tStartRefresh')  # time at next scr refresh
                    # update status
                    right_outcome.status = STARTED
                    right_outcome.setAutoDraw(True)
                
                # if right_outcome is active this frame...
                if right_outcome.status == STARTED:
                    # update params
                    pass
                
                # if right_outcome is stopping this frame...
                if right_outcome.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > right_outcome.tStartRefresh + 1.5-frameTolerance:
                        # keep track of stop time/frame for later
                        right_outcome.tStop = t  # not accounting for scr refresh
                        right_outcome.frameNStop = frameN  # exact frame index
                        # update status
                        right_outcome.status = FINISHED
                        right_outcome.setAutoDraw(False)
            ...
            for thisComponent in outcomeComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            thisExp.addData('outcome.stopped', globalClock.getTime())
            # Run 'End Routine' code from outcome_code
            #reset opacities
            left_outcome.opacity = 1
            right_outcome.opacity = 1
            not_win.opacity = 1
            # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
            if routineForceEnded:
                routineTimer.reset()
        ...
        # Run 'Begin Routine' code from pause_code
        ...
        pause_text.setText(feedback)
        # keep track of which components have finished
        pauseComponents = [pause_text]
        ...
        while continueRoutine and routineTimer.getTime() < 7.0:
            ...
            if pause_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > pause_text.tStartRefresh + 7-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    # completed 5.0 repeats of 'trials_block'
